...
type UserService interface {
	...
	AreFriends(userA *primitive.ObjectID, userB *primitive.ObjectID) bool
}
...
func (us *UserServiceImpl) AreFriends(userA *primitive.ObjectID, userB *primitive.ObjectID) bool {
	ctx := context.Background()
	db := us.Db.Database("wrapup-base")
	friendsColl := db.Collection("friends")

	// Check if there is a friend relationship between the two users
	count, err := friendsColl.CountDocuments(ctx, bson.M{
		"$or": []bson.M{
	...
	if err != nil {
		log.Println("Error while checking friend relationship: ", err)
		return false
	}
	return count > 0
}
